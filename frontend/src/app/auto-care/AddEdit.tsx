...
const AddEditAutoCare = () => {
  ...
  useEffect(() => {
    ...
    const loadAutoCare = async () => {
      setIsLoading(true);
      try {
        const response = await fetchAutoCareById(parseInt(autoCareID));
        if (response) {
          setAutoCare(response);

          // Set form values from response
          autoCareForm.setValue("customerId", String(response.customerID), {
            shouldValidate: true,
            shouldDirty: false,
          });
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const rq: AutoCare = {
      customerID: !isNew ? autoCare?.customerID : parseInt(data.customerId),
      vehicleID: !isNew ? autoCare?.vehicleID : parseInt(data.vehicleId),
      jobID: !isNew ? autoCare?.jobID : parseInt(data.jobId),
