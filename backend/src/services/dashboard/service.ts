...
export const getTotalCustomers = async (): Promise<{
    ...
    const totalCustomerQuery = await prisma.$queryRaw<
    ...
    const totalCustomerForThisMonthQuery = await prisma.$queryRaw<
    ...
    const totalCustomer = Number(totalCustomerQuery[0]?.active_customers ?? 0);
    const totalCustomersForCurrentMonth = Number(totalCustomerForThisMonthQuery[0]?.customers_this_month ?? 0);

    logger.info(`End: Fetched total customers ${totalCustomer} & customers for current month ${totalCustomersForCurrentMonth} `);
...
export const getTotalVehicles = async (): Promise<{
    ...
    const totalVehiclesQuery = await prisma.$queryRaw<
    ...
    const totalVehiclesForThisMonthQuery = await prisma.$queryRaw<
